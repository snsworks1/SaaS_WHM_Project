...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-2xl shadow-md">
    <h2 class="text-2xl font-bold mb-6">플랜 수정</h2>
    ...
    <form method="POST" action="{{ route('admin.plans.update', $plan->id) }}">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="ftp_accounts" class="w-full border rounded p-2" value="{{ old('ftp_accounts', $plan->ftp_accounts) }}" required>
            </div>

            <div>
                <label class="block font-medium mb-1">이메일 계정 수</label>
                <input type="number" name="email_accounts" class="w-full border rounded p-2" value="{{ old('email_accounts', $plan->email_accounts) }}" required>
            </div>

            <div>
                <label class="block font-medium mb-1">DB 수</label>
                <input type="number" name="sql_databases" class="w-full border rounded p-2" value="{{ old('sql_databases', $plan->sql_databases) }}" required>
            </div>

            <div>
                <label class="block font-medium mb-1">메일링 리스트 수</label>
                <input type="number" name="mailing_lists" class="w-full border rounded p-2" value="{{ old('mailing_lists', $plan->mailing_lists) }}" required>
            </div>

            <div>
                <label class="block font-medium mb-1">시간당 이메일 발송 제한</label>
                <input type="number" name="max_email_per_hour" class="w-full border rounded p-2" value="{{ old('max_email_per_hour', $plan->max_email_per_hour) }}" required>
            </div>

            <div>
                <label class="block font-medium mb-1">이메일당 최대 용량 (MB)</label>
                <input type="number" name="email_quota" class="w-full border rounded p-2" value="{{ old('email_quota', $plan->email_quota) }}" required>
            </div>
