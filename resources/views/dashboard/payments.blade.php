        ...
        <div class="max-w-7xl mx-auto bg-white p-6 shadow rounded-lg">
            ...
            <form method="GET" class="mb-6 flex flex-wrap gap-4">
                ...
                <div class="flex items-end">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">적용</button>
                </div>
            </form>
    ...
    <span class="px-2 py-1 text-xs font-semibold rounded {{ $badgeClass }}">
        {{ $typeLabel }}
            ...
            <td class="px-4 py-2">{{ $payment->plan->name ?? '-' }}</td>
                        <td class="px-4 py-2">{{ $payment->service->whm_domain ?? '-' }}</td> <!-- ✅ 도메인 출력 -->
